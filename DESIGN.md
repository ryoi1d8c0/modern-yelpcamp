...
**次の一手**: Phase 2 (5〜6) — ~~Honoプロジェクト作成~~(完了) → ~~ESLint/Prettier設定~~(完了) → Docker + PostgreSQL → Drizzle導入
...
| 2026-08-15 | backendのtypescriptを7系から5.x系(安定版)にダウングレード | `create-hono`が生成したTS7(ネイティブ実装プレビュー版)は`typescript-eslint`のpeer dependency(`<6.1.0`)と衝突するため。TS7はまだプレビュー段階でエコシステム対応が未成熟 |
| 2026-08-15 | backendのESLintは`defineConfig()`(ESLint本体、frontendと同じ方式)+ `typescript-eslint`recommendedで構成し、Prettierと役割分担する(`eslint-config-prettier`でスタイル系ルールを無効化) | `tseslint.config()`はtypescript-eslint側で非推奨化されたため。Lintとフォーマットの責務を分離するのが現代的なTSプロジェクトの定番構成 |
| 2026-08-15 | backendの`tsconfig.json`に`noUnusedLocals`・`noUnusedParameters`・`noUncheckedIndexedAccess`を追加 | デフォルトの`strict: true`だけでは検出できない未使用変数や配列アクセスのundefined可能性をコンパイル時に検出するため |
